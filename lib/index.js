...
ScpSync.prototype.init = function(cb) {
  ...
  if (program.sourcepath) {
    ...
    console.error("no source specified", settings.sourcepath);
    return 1;
  }
...
ScpSync.prototype.transfer = function(filepath) {
  var self = this;
  log('[note] transfer: ', settings.sourcepath + filepath);
...
ScpSync.prototype.remove = function(filepath) {
  ...
  settings.destinationpath = jsesc(settings.destinationpath + filepath, {
    'quotes': 'double',
    'wrap': true
  });
  var cmd = 'rm -r ' + settings.destinationpath;
  log('remove cmd: ', cmd);
